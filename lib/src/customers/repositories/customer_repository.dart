...
CustomerRepository customerRepo(CustomerRepoRef ref) {
  return CustomerRepository(
    ...
    toSycnDataDao:
        ref.watch(appDbProvider.select((prov) => prov.toSycnDataDao)),
  );
}
...
class CustomerRepository {
  ...
  Future<List<CustomerWithRegistration>> searchScreening(
      Screening screening, String search) {
  ...
  Future<Customer?> findByMobileNo(
    ...
    return await customerDao.findByMobileNo(
        mobileNo: mobileNo, excludeId: excludeId);
  }
  ...
  Future<bool> update(Customer customer, {Expression<bool>? where}) async {
    return await db.transaction(() async {
      final result = await customerDao.updateCustomer(customer.toData(),
          where ?? db.customersTable.id.equals(customer.id!));
