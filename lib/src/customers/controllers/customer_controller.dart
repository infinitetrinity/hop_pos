...
class CustomerController extends _$CustomerController {
  ...
  Future<bool> isMobileNoTaken(
      {required String mobileNo, int? excludeId}) async {
    final repo = ref.read(customerRepoProvider);
    final customer =
        await repo.findByMobileNo(mobileNo: mobileNo, excludeId: excludeId);
  ...
  Future<bool> updateCustomer(CustomerForm form) async {
    ...
    final dynamic repo = customer.isNew
        ? ref.read(newCustomerRepoProvider)
